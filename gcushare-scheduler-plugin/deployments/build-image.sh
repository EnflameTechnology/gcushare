...
Options:
  ...
  --version   Required. Specify the version of gcushare, v1 for k8s1.24+, v1beta1 for k8s1.18, k8s 1.20 and k8s1.22. Read usr guide for the detail.
...
function checkCommand() {
    ...
    case "$VERSION" in
        ...
        "v1" | "v1beta1")
            ...
            ;;
        *)
            echo -e "\033[31mError: Invalid version '$VERSION'. Supported versions: v1, v1beta1\033[0m"
            usage
            exit 1
            ;;
    esac
...
function buildImage() {
    ...
    if [ "$CLI_NAME" == "nerdctl" ]; then
        $CLI_NAME --namespace $NAMESPACE build --build-arg VERSION=$VERSION -t $REPO_FULL_NAME --file dockerfiles/Dockerfile.$OS .
    elif [ "$CLI_NAME" == "ctr" ]; then
        docker build --build-arg VERSION=$VERSION -t $REPO_FULL_NAME --file dockerfiles/Dockerfile.$OS .
    else
        $CLI_NAME build --build-arg VERSION=$VERSION -t $REPO_FULL_NAME --file dockerfiles/Dockerfile.$OS .
    fi
